...
var S = {
  ...
  typeFilter: 'all',   // 'all' | 'vat' | 'ticket' | 'toll' | 'nontax'
  formatFilter: 'all', // 'all' | 'pdf' | 'ofd' | 'image' | 'xml'
...
async function processFileList(fileList) {
  ...
  for (var fdIdx = 0; fdIdx < fileList.length; fdIdx++) {
    ...
    if (phIdx >= 0 && r) {
      ...
      finalizeMedicalDetailPages(items); // 加入列表后立即排除医疗明细页附件（幂等）
      added += items.length;
    } else if (phIdx >= 0) {
...
function openSlotContextMenu(e) {
  ...
  selectSlot(idx);
  _ctxIdx = S.files.indexOf(f); // 统一菜单：份数/旋转/删除/复制同样按此文件操作
...
// 预览区槽位右键动作（单票调整，同步桌面版 #33；菜单与列表右键统一，目标文件由 selectSlot/_ctxIdx 指向）
...
// 右键分发：列表项/预览槽位弹自定义菜单（内容不同）；输入框保留原生菜单（复制/粘贴）；其余区域屏蔽浏览器默认菜单
...
// 在共享批次的主票上挂附件计数徽章，toast 明示排除行为。
// PDF 文字层完成后触发；_medDetailHandled 防空转。主票用 srcPdfBytes 引用分组 + srcPageIndex 排序。
function finalizeMedicalDetailPages(results) {
  ...
  results.forEach(function(r) {
    ...
    detailCount++;
    var main = null;
    // 同一 srcPdfBytes（同源 PDF）中的主票 = 首个非明细页
    if (r.srcPdfBytes) {
      var same = S.files.filter(function(o) { return o !== r && o.srcPdfBytes === r.srcPdfBytes && !o._isMedicalDetail; });
      same.sort(function(a, b) { return (a.srcPageIndex || 0) - (b.srcPageIndex || 0); });
      main = same[0] || null;
    }
    if (main) {
  ...
  if (detailCount > 0) {
    updateDuplicateMarks();
    toast((mainName ? '“' + mainName + '”的' : '') + detailCount + ' 页收费明细已自动排除，仅保留汇总页');
  }
}
...
function canListDrag() { return S.fileFilter === 'all' && S.printedFilter === 'all' && S.typeFilter === 'all' && S.formatFilter === 'all'; }
...
function selectSlot(idx) {
  ...
  if (idx >= 0) {
    ...
    // 首次选中提示隐藏手势（同步桌面版 #33）：双击重置 / 右键菜单，跨会话仅提示一次
...
// 选中槽位浮动工具条：锚定槽位上方居中，随内容滚动（同步桌面版 #33）
...
// 重置全部单票调整（同步桌面版 #33：与「应用到全部」对应的反向批量入口）
