...
本段记录在本机仓库对源码/构建做的临时改动，不属于上游发布版本。

### ✨ UI 改动

- **全选/取消全选合并为单个切换按钮**
  - `src/index.html`：原「☑ 全选」「◻ 取消全选」两个按钮合并为 `#selectToggleBtn`，点击调用 `toggleSelectAll()`
  - `src/app.js`：新增 `toggleSelectAll()`（已全选则取消、否则全选）与 `updateSelectToggle()`（按勾选状态同步按钮文案「全选」/「取消全选」及激活高亮）；在 `renderFileList()` 中调用，单选/反选任意发票后按钮状态自动刷新
  - `src/styles.css`：新增 `.ib.active` 激活态与 `#selectToggleBtn` 自适应宽度样式
- **文件操作区四个按钮换 PNG 图标**
  - 排版份数 `②` → `icons/copy_layout_32.png`
  - 一键识别 `🔍` → `icons/ocr_32.png`
  - 按日期排序 `📅` → `icons/date_order_32.png`
  - 删除 `🗑` → `icons/delete_32.png`
  - 图标源文件位于 `src/icons/`（前端从 `src/` 打包，必须放此处才能被 `<img>` 加载）
  - `src/styles.css`：新增 `.ib-icon`（16px、居中、禁止指针事件，避免遮挡点击）
  - `src/app.js`：`updateOcrAllBtn()` 空闲态原为 emoji 重置按钮，已改为恢复 OCR 图标；OCR 进行中仍显示进度 spinner
- **工具栏按钮换 PNG 图标 + 新增复原布局功能**
  - 清空所有发票 `🗑` → `icons/empty_files_32.png`
  - 排版设置 `⚙` → `icons/layout_settings_32.png`
  - **新增「复原布局」按钮**（`icons/reset_layout_32.png`），位于排版设置右侧，hover 提示「恢复默认排版布局」
  - `src/app.js`：新增 `resetLayout()` — **保持当前选中的 M×N 网格、纸张规格/方向与边距/间距不变**，仅清除所有发票的手动槽位调整（`slotScale=1`、`slotOffsetX/Y=0`），回到刚添加完文件的干净对齐状态；不删除发票、不影响其他设置

### 🔧 构建脚本

- 新增 `build_exe.bat`：设置 MSVC 环境（`vcvars64.bat`）+ 加入 cargo 到 PATH → `npm install` → `npm run build`（轻量版）
- 新增 `build_ocr.bat`：同上，最后一步改为 `npm run build:ocr`（OCR 版，含 PP-OCRv5）
- **NSIS 安装包图标**：`src-tauri/tauri.conf.json` 的 `bundle.windows.nsis` 新增 `installerIcon: "icons/icon.png"`（基础配置，OCR 版经 `tauri.ocr.conf.json` 继承同样生效）
- 用途：在 PowerShell/CMD 中一键编译桌面 exe，免去手动配置 MSVC 链接器环境
- 说明：Tauri 在 Windows 用 MSVC 编译，必须先 `call vcvars64.bat` 设置环境；直接 `npm run build:ocr` 会报 `cargo: program not found`
