...
- **多格式支持**：PDF、OFD、XML 数电票、JPG、PNG、BMP
...
- **三维正交筛选**（v2.5.2，v2.6.3 / v2.6.7 升级）：类型（专票/普票/车票/通行费/医疗/非税/其他）× 格式（PDF/OFD/图片/XML）× 状态（全部/未打印/已打印/重复）自由组合，类型匹配统一走识别单一真源；全选遵循当前筛选——筛选「车票」→全选→2×2 打印，再筛选「专票」→全选→2×1 打印，多步实现分类分批排版；工具栏与筛选区固定在列表顶部不随滚动，筛选区可折叠
...
- **单票浮动工具条**（v2.6.1，v2.6.4 加入旋转，v2.6.8 钳制 + 一键删除，v2.6.9 按格子归位）：选中版面发票后浮出操作条——重置 / 居中 / 应用到全部 / 旋转（实时显示当前角度，如 `↻ 90°`）/ 删除，随内容滚动跟手；本格子上方若被别的格子占用（如下排格子的上方正是上排票面）则自动贴回本格子内侧，格子过窄时只留图标
...
- **多页发票逻辑票聚合**（v2.6.7）：同批次同一 PDF 的多页且发票号一致视为一张跨页票——列表 / 方格双视图显示「续 n/N」「共 N 页」徽章，统计只计合计页金额，去重按整组计数与删除，杜绝续页被误删导致打印缺页
- **重复发票识别与去重**（v2.2.2 / v2.3.1 / v2.6.7）：按发票号自动检测可靠重复并支持「添加时自动去重」与「重复」筛选一键勾选删除，计数与删除按逻辑票（跨页票只算一张）；无发票号时按 销售方+金额+日期 仅作 ⚠ 疑似标记，不自动删除，避免误删同日同额的真发票
...
- 裁切线、编号标记、边框显示、自定义水印
- **裁剪白边**（v2.6.2 完善）：自动裁掉发票图片内部的纯白边，留边 0–60 可配置，PDF 矢量裁切与预览一致
- **文本增强**（v2.2.0）：浅色/模糊图片发票一键增强（色阶拉伸 + 中间调压暗 + USM 锐化），全程本地处理，红章不偏色
- **副本标记**（v2.6.0）：开启后同发票多副本在槽位左上角显示 n/N 序号，跨页也能核对
- 金额统计、票种标签（专票/普票/车票/通行费/非税同形异色 chip）、发票类型自动检测
- **页脚**：打印页码（第 X 页 / 共 Y 页）、打印日期、自定义页脚文本，独立下边距控制

### 📊 数据导出

- **发票汇总表**（v2.0.3，v2.6.7 口径修正）：报销必备，一键导出所有发票明细，字段可勾选（14 项），金额/名称等可直接编辑修正，合计行自动汇总含税/不含税/税额，CSV 格式 Excel 直接打开，列选择和备注持久化记忆；多页发票只计合计页金额，不再每页累加（issue #40）
...
| 图像处理 | image 0.25 (Rust) | JPG/PNG/BMP 解码（v2.6.3 裁掉 WebP/TIFF/ICO 无用格式，安装包 −28%），kamadak-exif 方向自动修正 |
...
- [x] 通行费发票识别 + 版面拖拽排序（v2.5.0）
- [x] 列表拖拽排序 + 拖尾空槽临时占位留白 + OFD 渲染系列修复（v2.5.1）
- [x] 三维正交筛选 + 旋转导出语义修复 + 更新提示修复（v2.5.2）
- [x] 右键菜单 + 副本标记 + 忽略所有更新（v2.6.0）
- [x] 右键菜单两处统一 + 浮动工具条 + 防误触 + 恢复默认拆档（v2.6.1）
- [x] 裁剪白边进 PDF（矢量裁切）+ 留边可配置 + 全链路口径统一（v2.6.2）
...
- [x] 粘贴单格式 + 清晰度体检/打印自动增强 + 单票缩放 5x + 白边裁剪打印一致性修复（v2.6.6）
