...
- **三维正交筛选**（v2.5.2）：类型（发票/车票/通行费/财政）× 格式（PDF/OFD/图片/XML）× 状态（全部/未打印/已打印/重复）自由组合，全选遵循当前筛选——筛选「车票」→全选→2×2 打印，再筛选「发票」→全选→2×1 打印，两步实现分类分批排版；筛选区可折叠
...
- **右键菜单**（v2.6.0，v2.6.1 两处统一）：文件列表与预览槽位共用同一份完整菜单——排版份数、旋转 90°、OCR 识别、复制发票信息、单票调整（重置此票 / 居中 / 应用到全部 / 重置全部调整）、删除；右键同步选中态
...
- **单票独立调整**（v1.9.0+，v2.0.1-v2.0.2, v2.0.8, v2.6.1 增强）：每张发票预览拖拽移动 + 角落 handle 缩放，九宫格快速对齐，滚轮微调 + 滚轮单票缩放（5%/步），±150mm 偏移范围，拖拽约束动态化，放大上限 3x，编辑态溢出预览，打印区域虚线标识，双击重置，选中票面浮现工具条（重置/居中/应用到全部），调整参数可选持久化记忆，侧边栏「单票调整」面板或发票弹窗参数编辑，PDF 按参数裁剪输出
- **防误触**（v2.6.1）：侧栏滚轮增减改为焦点门槛——点击激活输入框/滑块后才响应滚轮，滚动浏览侧栏不再误改设置；「恢复默认」拆为「仅恢复版面」与「恢复全部」两档；首次选中票面提示双击重置手势

### ✂️ 辅助功能

- 裁切线、编号标记、边框显示、裁剪白边、自定义水印
- **副本标记**（v2.6.0）：开启后同发票多副本在槽位左上角显示 n/N 序号，跨页也能核对
