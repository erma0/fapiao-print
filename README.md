...
- **字段提取**: PDF 文字层 / OFD XML / XML 数电票自动提取发票信息（增值税 / 车票 / 非税票据 / 通行费 / 网约车）
- **重复检测**: 发票号 / 销售方+金额+日期双 key 识别重复发票，⚠标记 + 一键勾选删除
- **多张拼版**: 自定义 1-10×1-10 网格,A4/A5/B5/Letter/Legal/自定义纸张
- **完整排版控制**: 边距、间距、缩放、旋转、切割线、边框、水印、页码、页脚
- **单票独立调整**: 拖拽移动 + 滚轮缩放 (5%/步) + 九宫格对齐 + 调整记忆
- **版面拖拽排序**: 拖到发票边缘 = 顺位插入,拖到中间 = 两张对调
- **预览滚轮交互**: 普通滚轮翻页 (触底/触顶),Ctrl+滚轮缩放视图,选中槽位滚轮缩放单票
- **列表双向联动**: 点击列表项跳页选中槽位,选中槽位反向高亮定位列表
- **文件列表双视图**: 列表 / 方格缩略图卡片一键切换
...
- **离线**: 无需网络,全浏览器运行
- **设置持久化**: 排版参数/水印/页脚/汇总列/视图偏好等自动记忆
- **主题切换**: 浅色/深色
...
- **v3.4.0** (本分支): 纯前端,零后端零依赖 — 🕸️ 在线试用 [fapiao.erma0.cn](https://fapiao.erma0.cn)
- **v2.5.1** ([`master` 分支](https://github.com/erma0/fapiao-print/tree/master)): Tauri 桌面版,Rust 后端 + PDFium 引擎,支持 OCR / 静默打印 / 批量重命名 / 报销单分段

## ⚠️ 与桌面版的差异

纯 Web 版因浏览器安全限制，以下功能不可用或未实现：
- **OCR 识别**: 浏览器无法本地运行 PP-OCR 引擎,图片发票的字段需手动填写
- **批量重命名**: 无法访问本地文件系统重命名文件
- **打印机选择 / 静默打印**: 浏览器无法指定打印机,必须经过打印对话框
- **文件列表记忆**: 无法持久化本地文件路径跨会话恢复
- **报销单分段模式**: 尚未移植 (依赖布局引擎扩展)
- **槽位精准上传 / 版面留白**: 尚未移植
- **快捷版面管理**: 顶部工具栏为固定 6 个快捷布局,暂不支持自定义管理
- **文本增强**: 尚未移植 (需 Canvas 版算法重写)
