...
function _normTextForExtract(text) {
  ...
  // Fix spurious decimal points inserted by pdf_oxide between digit groups.
  // Pattern: "3.99.00" → "399.00", "2.5412..." → "25412...", "2.025年" → "2025年"
  // Rule: when a number has multiple dots, keep only the last dot (if followed by 2 digits)
  // or remove all dots (for integers like invoice numbers, years, credit codes).
  // Pass 1: Fix amounts with multi-dot pattern like "3.99.00" or "¥3.53.10"
  // The last ".XX" is the real decimal part; earlier dots are spurious.
  text = text.replace(/(\d[\d,]*)\.(\d+)\.(\d{2})(?!\d)/g, function(m, a, b, c) {
    return a + b + '.' + c;
  });
  // Pass 2: Fix 3+ dots in a number chain like "1.2.3.45" → "123.45"
  for (var _dotIter = 0; _dotIter < 3; _dotIter++) {
    var _dotPrev = '';
    while (_dotPrev !== text) {
      _dotPrev = text;
      text = text.replace(/(\d[\d,]*)\.(\d+)\.(\d{2})(?!\d)/g, '$1$2.$3');
    }
  }
  // Pass 3: Fix year-like patterns "2.025年" → "2025年", "2.025-06-11" → "2025-06-11"
  text = text.replace(/(\d)\.(\d{3})(\s*年)/g, '$1$2$3');
  text = text.replace(/(\d)\.(\d{3})(\s*-)/g, '$1$2$3');
  // Pass 4: Fix invoice-number-like patterns "2.5412000000133760562" → "25412000000133760562"
  // Only when preceded by "发票号码：" or "票据号码：" (8+ digits after dot)
  text = text.replace(/(发\s*票\s*号\s*码|票\s*据\s*号\s*码)[:：\s]*(\d+)\.(\d{8,})/g, '$1:$2$3');
  // Pass 5: Fix credit code patterns "9.1 3 2 0 2 0 0 7 9 6 1 4 8 3 6 8 W"
  // Already handled by digit-space collapse above, but also fix "9.13 2 0 2 0 0" → "91320200"
  text = text.replace(/(统一社会信用代码|纳税人识别号)[:：\s]*(\d)\.\s*(\d)/g, '$1:$2$3');
  // Pass 6: Fix ¥ amounts with spurious dots like "¥3.99.00" → "¥399.00"
  text = text.replace(/¥\s*(\d[\d,]*)\.(\d+)\.(\d{2})(?!\d)/g, function(m, a, b, c) {
    return '¥' + a + b + '.' + c;
  });
...
/**
 * Split a fused company name string into two separate company names.
 * When pdf_oxide fuses left/right column text into a single span,
 * two company names can appear concatenated (e.g., "公司A公司B").
 * Strategy: find company suffix boundaries and split after the first one.
 * Returns an array of 1 or 2 strings.
 */
function _splitFusedCompanyNames(text) {
  if (!text) return [text];
  var suffixes = [
  ...
  var shortSuffixes = ['厂', '店', '部', '院', '所', '中心', '馆', '站', '社', '行', '会'];
  for (var si = 0; si < suffixes.length; si++) {
    var sfx = suffixes[si];
    var idx = text.indexOf(sfx);
    while (idx >= 0) {
      var end = idx + sfx.length;
      if (end < text.length) {
        var after = text.substring(end);
        if (/[\u4e00-\u9fffA-Z0-9]/.test(after.charAt(0))) {
          return [text.substring(0, end), after];
        }
      }
      idx = text.indexOf(sfx, end);
    }
  }
  for (var si2 = 0; si2 < shortSuffixes.length; si2++) {
    var sfx2 = shortSuffixes[si2];
    var idx2 = text.indexOf(sfx2);
    while (idx2 >= 0) {
      var end2 = idx2 + sfx2.length;
      if (end2 < text.length) {
        var after2 = text.substring(end2);
        if (/[\u4e00-\u9fffA-Z0-9]/.test(after2.charAt(0)) && idx2 >= 2) {
          return [text.substring(0, end2), after2];
        }
      }
      idx2 = text.indexOf(sfx2, end2);
    }
  }
  var ccRegex = /([0-9][A-Z0-9]{17})([A-Z0-9])/;
  var ccMatch = text.match(ccRegex);
  if (ccMatch && ccMatch.index >= 0) {
    var ccEnd = ccMatch.index + 18;
    return [text.substring(0, ccEnd), text.substring(ccEnd)];
  }
  return [text];
}
...
function _cleanName(raw) {
  ...
  // Strip page number prefix (e.g., "共1页 第1页茅箭区广汇广告制作工作室" → "茅箭区广汇广告制作工作室")
  name = name.replace(/^共\s*\d+\s*页\s*第\s*\d+\s*页\s*/, '');
  // Strip date suffix (e.g., "无锡天鹏菜篮子工程有限公司开票日期:2026-04-28" → "无锡天鹏菜篮子工程有限公司")
  name = name.replace(/开\s*票\s*日\s*期[:：].*$/, '');
  // Strip receipt unit suffix (e.g., "（章）:复核人:陆欣收款人:李娟" is not a company name)
  name = name.replace(/[（(]\s*章\s*[）)][：:].*$/, '');
  // Strip trailing "出行人..." or other table header contamination
  name = name.replace(/出行人.*$/, '');
  name = name.replace(/有效身份证件号.*$/, '');
  ...
  // Skip names that are just receipt/signature fragments (e.g., "（章）:复核人:陆欣收款人:李娟")
  if (/^[（(]\s*章\s*[）)]/.test(name)) return '';
  // Skip names that start with receipt-related keywords
  if (/^(?:收款单位|复核人|收款人|开票人)/.test(name)) return '';
...
function _extractNamesByCoords(words, result) {
  ...
  for (var _ilni = 0; _ilni < nameLabels.length; _ilni++) {
    ...
    if (_ilnMatch && _ilnMatch[1].trim()) {
      var _ilnRaw = _ilnMatch[1];
      var _ilnSplit = _splitFusedCompanyNames(_ilnRaw);
      if (_ilnSplit.length === 2) {
        var _leftName = _cleanName(_ilnSplit[0]);
        var _rightName = _cleanName(_ilnSplit[1]);
        if (_leftName) {
          inlineNameResults.push({ label: _ilnWord, name: _leftName, ny: _ilnWord.ny, nx: _ilnWord.nx, wordIndex: words.indexOf(_ilnWord), isLeftSide: true });
        }
        if (_rightName) {
          inlineNameResults.push({ label: _ilnWord, name: _rightName, ny: _ilnWord.ny, nx: _ilnWord.nx, wordIndex: words.indexOf(_ilnWord), isLeftSide: false });
        }
      } else {
        var _ilnName = _cleanName(_ilnRaw);
        if (_ilnName) {
  ...
  for (var li = 0; li < nameLabels.length; li++) {
    ...
    for (var wi = 0; wi < words.length; wi++) {
      ...
      if (isLeftSide && w.nx >= 0.5) continue;
      if (!isLeftSide && w.nx < 0.5) continue;
      ...
      var isNotCreditCodeWord = !(_wCleaned.length >= 15 && _wCleaned.length <= 40 && /^[0-9]/.test(_wCleaned) && /[A-Z]/.test(_wCleaned));
      // Filter out table header terms that appear on the seller side (ride invoices)
      var isNotTableHeader = !/^(?:出行人|有效身份证件号|出行日期|出发地|到达地|交通工具类型)/.test(w.text) && !/^(?:出行人|有效身份证件号|出行日期|出发地|到达地|交通工具类型)/.test(w.normText);

      if (isRightOfLabel && isInYRange && isNotLabel && isNotCreditLabel && isNotSectionLabel && isNotMetadata && isNotCreditCodeWord && isNotTableHeader) {
    ...
    // Check if any region word contains fused company names and split them
    var splitRegionWords = [];
    for (var rwi = 0; rwi < regionWords.length; rwi++) {
      var rw = regionWords[rwi];
      var rwText = rw.text || rw.normText;
      var rwSplit = _splitFusedCompanyNames(rwText);
      if (rwSplit.length === 2) {
        var rwLeft = _cleanName(rwSplit[0]);
        var rwRight = _cleanName(rwSplit[1]);
        if (rwLeft && rwRight) {
          splitRegionWords.push({ text: rwLeft, isLeftPart: true, word: rw });
          splitRegionWords.push({ text: rwRight, isLeftPart: false, word: rw });
        } else {
          splitRegionWords.push({ text: rwText, isLeftPart: null, word: rw });
        }
      } else {
        splitRegionWords.push({ text: rwText, isLeftPart: null, word: rw });
      }
    ...
    // If we got split words, handle them specially
    var hasSplitWords = splitRegionWords.some(function(sw) { return sw.isLeftPart !== null; });
    if (hasSplitWords) {
      for (var swi = 0; swi < splitRegionWords.length; swi++) {
        var sw = splitRegionWords[swi];
        var swCleaned = _cleanName(sw.text);
        if (!swCleaned) continue;
        if (sw.isLeftPart === true) {
          foundNames.push({ label: label, name: swCleaned, ny: label.ny, nx: label.nx, wordIndex: words.indexOf(label), isLeftSide: true });
        } else if (sw.isLeftPart === false) {
          foundNames.push({ label: label, name: swCleaned, ny: label.ny, nx: label.nx, wordIndex: words.indexOf(label), isLeftSide: false });
        } else {
          foundNames.push({ label: label, name: swCleaned, ny: label.ny, nx: label.nx, wordIndex: words.indexOf(label), isLeftSide: isLeftSide });
        }
      }
      continue;
    }
...
function _extractByText(fullText, words) {
  ...
  if (words && words.length > 0) {
    ...
    words.forEach(function(w) {
      ...
      // Handle fused credit codes: two 18-char codes concatenated (e.g., "91320200796148368W91230110MA19K95R36")
      if (cleaned.length >= 36 && /^[0-9]/.test(cleaned)) {
        var code1 = cleaned.substring(0, 18);
        var code2 = cleaned.substring(18, 36);
        if (/^[0-9][A-Z0-9]{17}$/.test(code1) && /^[0-9][A-Z0-9]{17}$/.test(code2)) {
          if (w.nx < 0.5) {
            if (!coordCodes.buyer) coordCodes.buyer = code1;
            if (!coordCodes.seller) coordCodes.seller = code2;
          } else {
            if (!coordCodes.seller) coordCodes.seller = code1;
          }
          return;
        }
      }
...
function _extractAmountsByText(fullText) {
  ...
  // Detect non-tax invoices (no tax amount applicable)
  var isNonTaxInvoice = /非税收入.*票据/.test(text) || /金\s*额\s*合\s*计/.test(text) && !/价\s*税\s*合\s*计/.test(text) && !/税\s*额/.test(text);
  ...
  // For non-tax invoices, no tax breakdown needed
  if (isNonTaxInvoice && result.amountTax > 0) {
    result.amountNoTax = result.amountTax;
    result.taxAmount = 0;
    console.log('[非税发票] 含税价=' + result.amountTax + ', 无税额');
    return result;
  }
  ...
  if (!sellerCreditCode) {
    ...
    for (var ci = 0; ci < ccLabels.length && !sellerCreditCode; ci++) {
      ...
      var codeWords = words.filter(function(w) {
        ...
        var cText = w.normText.replace(/[^A-Z0-9]/g, '');
        return /^[0-9][A-Z0-9]{14,19}$/.test(cText) || (cText.length >= 36 && /^[0-9][A-Z0-9]+$/.test(cText));
      });
      if (codeWords.length > 0) {
        codeWords.sort(function(a, b) {
          return Math.abs(a.cx - ccLabels[ci].cx) - Math.abs(b.cx - ccLabels[ci].cx);
        });
        var _cwText = codeWords[0].normText.replace(/[^A-Z0-9]/g, '').toUpperCase();
        if (_cwText.length >= 36) {
          var _cwPart = _cwText.substring(18, 36);
          if (/^[0-9][A-Z0-9]{17}$/.test(_cwPart)) sellerCreditCode = _cwPart;
        } else {
          sellerCreditCode = _cwText;
        }
      }
    }
  }
...
function extractByCoordinates(ocrResult) {
  ...
  if (!sellerCreditCode || !buyerCreditCode) {
    ...
    if (words && words.length > 0) {
      ...
      words.forEach(function(w) {
        var cleaned = w.normText.replace(/[^A-Za-z0-9]/g, '').toUpperCase();
        // Handle fused credit codes (36+ chars = two 18-char codes)
        if (cleaned.length >= 36 && /^[0-9]/.test(cleaned)) {
          var fc1 = cleaned.substring(0, 18);
          var fc2 = cleaned.substring(18, 36);
          if (/^[0-9][A-Z0-9]{17}$/.test(fc1) && /^[0-9][A-Z0-9]{17}$/.test(fc2)) {
            if (w.nx < 0.5) {
              if (!coordFallback.buyer) coordFallback.buyer = fc1;
              if (!coordFallback.seller) coordFallback.seller = fc2;
            } else {
              if (!coordFallback.seller) coordFallback.seller = fc1;
            }
            return;
          }
        }
