...
function openExternal(url) {
  if (isTauri && __api) {
    __api.call('open_url', { url: url }).catch(function(e) {
...
async function downloadSumatraPdf() {
  ...
  try {
    if (isTauri && window.__TAURI_INTERNALS__) {
      ...
      var eventId = await __api.call('plugin:event|listen', {
        event: 'sumatra-download-progress',
        target: { kind: 'Any' },
        handler: callbackId
      });
      unlistenProgress = function() {
        try { __api.call('plugin:event|unlisten', { event: 'sumatra-download-progress', eventId: eventId }); } catch(e) {}
      };
    }
    var result = await __api.call('download_sumatrapdf');
...
function cancelSumatraDownload() {
  ...
  if (isTauri && __api) { try { __api.call('cancel_download'); } catch(e) {} }
  var modal = document.getElementById('sumatraPdfModal');
  if (modal) modal.classList.add('hidden');
  toast('下载已取消');
}
...
async function downloadPdfiumDll() {
  ...
  try {
    if (isTauri && window.__TAURI_INTERNALS__) {
      ...
      unlistenProgress = function() {
        try { __api.call('plugin:event|unlisten', { event: 'pdfium-download-progress', eventId: eventId }); } catch(e) {}
      };
    }
    var result = await __api.call('download_pdfium_dll');
    if (unlistenProgress) unlistenProgress();
...
async function restoreFiles(paths) {
  ...
  var checks = await Promise.all(paths.map(function(p) {
    return __api.call('check_path_exists', { path: p })
  ...
  try {
    if (valid.length <= 3) {
      ...
      var fileDataList = await __api.call('open_invoice_files', { paths: valid });
...
async function triggerUpload() {
  if (isTauri && __api) {
    try {
      var result = await __api.call('plugin:dialog|open', {
        options: {
          multiple: true,
          title: '选择发票文件',
      ...
      if (paths.length <= 3) {
        ...
        var fileDataList = await __api.call('open_invoice_files', { paths: paths });
        if (fileDataList && fileDataList.length > 0) {
          await processFileDataList(fileDataList);
        } else {
          toast('无法读取所选文件');
...
async function processFilesIncremental(paths) {
  ...
  var fileDataMap = {};
  try {
    var allFileData = await __api.call('open_invoice_files', { paths: paths });
...
function applyOcrAsync(fileObj, dataUrl) {
  if (!hasOcr || !isTauri || !__api || window.__TAURI_CLOSING__) return;
...
function loadFileFromDataUrlFast(fd) {
  ...
  return new Promise(function(resolve) {
    ...
    else if (ext === 'ofd' && isTauri && __api && filePath) {
      __api.call('parse_ofd', { ofdPath: filePath }).then(function(result) {
        ...
        __api.call('open_ofd_images', { ofdPath: filePath }).then(function(fileDataList) {
          if (fileDataList && fileDataList.length > 0) {
    ...
    else if (ext === 'xml' && isTauri && __api && filePath) {
      __api.call('parse_xml_invoice', { xmlPath: filePath }).then(function(info) {
        var fileObj = createFileObj({
...
function ocrFile(i) {
  ...
  if (!isTauri || !__api) { toast('OCR 识别需要桌面版'); return; }
...
function switchTab(n, el) {
  ...
  if (n === 'print' && !_printersLoaded && isTauri && __api) {
...
async function verifyInvoice(backup) {
  ...
  if (isTauri && __api) {
    try { await __api.call('open_url', { url: url }); } catch(e) { toast('打开查验网站失败: ' + String(e)); }
...
// =====================================================
if (isTauri && __api) {
  __api.call('get_config').then(function(config) {
...
(function() {
  function showApp() {
    if (isTauri && __api) {
      ...
      __api.call('check_ocr_available').then(function(available) {
        hasOcr = !!available;
      ...
      __api.call('check_winrt_pdf').then(function(available) {
        _winrtPdfAvailable = !!available;
        if (!_winrtPdfAvailable) {
          console.warn('[PDF] WinRT PDF 组件不可用，将使用 PDFium fallback');
          __api.call('check_pdfium_available').then(function(pdfiumAvail) {
            if (!pdfiumAvail) {
              showPdfiumMissing('当前系统的 PDF 组件不可用，需要下载 PDFium 渲染引擎才能加载 PDF 文件。');
            }
          }).catch(function() {});
      ...
      try { __api.call('show_window'); } catch(e) {}
...
async function exportSummaryCsv() {
  ...
  if (isTauri && __api) {
    try {
      var defaultDir = '';
      try { defaultDir = await __api.call('get_downloads_dir'); } catch(e) {}
      var ts = new Date();
      ...
      var savePath = await __api.call('plugin:dialog|save', {
        options: {
          title: '保存汇总表',
      ...
      if (!savePath) return;
      await __api.call('write_text_file', { path: savePath, content: csvContent });
      closeSummaryModal();
      ...
      try { await __api.call('open_file', { path: dirPath }); } catch(e) {}
      toast('已保存: ' + savePath);
...
async function executeRename() {
  ...
  for (var i = 0; i < execList.length; i++) {
    ...
    try {
      var isBrowserMode = !isTauri || !__api;
      if (!isBrowserMode) {
        await __api.call('rename_file', { srcPath: srcPath, destPath: newPath });
      } else {
