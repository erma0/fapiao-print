...
// =====================================================
// Dependencies (global): isTauri, __api, S, getSettings, getActiveFiles, buildPages, showLoading, hideLoading, toast, getSaveDir, setSaveDir, escHtml, calculateLayout
 ...
 * Uses Tauri 2.x event system via __api.call('plugin:event|listen').
 * Returns an unlisten function.
 */
async function listenPdfProgress() {
  if (!isTauri || !__api) return null;
  try {
    ...
    return function() {
      try { __api.call('plugin:event|unlisten', { event: 'pdf-progress', eventId: eventId }); } catch(e) {}
    };
  } catch(e) {
...
async function doPdfiumPrint(files, s) {
  if (isTauri && __api) {
    try {
      var available = await __api.call('check_pdfium_available');
      if (!available) {
  ...
  if (canUseCachedPdf(currentRequest) && isTauri && __api) {
    try {
      ...
      var cacheResult = await __api.call('pdfium_print_pdf', {
        pdfPath: _lastPdfPath,
        printerName: s.printerName || null,
        copies: s.copies || 1,
        duplex: s.duplex || false,
  ...
  try {
    ...
    if (isTauri && __api) {
      ...
      var result = await __api.call('pdfium_vector_print', {
        request: layoutReq,
        printerName: s.printerName || null,
        copies: s.copies || 1,
        duplex: s.duplex || false
...
async function doPdfReaderPrint(files, s) {
  ...
  if (canUseCachedPdf(currentRequest) && isTauri && __api) {
    try {
      showLoading('正在使用缓存PDF打印...');
      var cacheResult = await __api.call('print_pdf_file', {
        pdfPath: _lastPdfPath,
        directPrint: true,
        printerName: s.printerName || null
      });
  ...
  try {
    ...
    if (isTauri && __api) {
      ...
      if (result.success) {
        ...
        try {
          var printResult = await __api.call('print_pdf_file', {
            pdfPath: result.pdfPath,
            directPrint: true,
            printerName: s.printerName || null
          });
...
async function savePdf() {
  ...
  if (isTauri && __api) {
    try {
      ...
      if (savedDir) {
        try {
          var checkResult = await __api.call('check_path_exists', { path: savedDir });
      ...
      if (savedDir && dirExists) {
        ...
        if (!defaultDir) {
          try { defaultDir = await __api.call('get_downloads_dir'); } catch(e) { /* ignore */ }
        }
        savePath = await __api.call('plugin:dialog|save', {
          options: {
            title: '保存发票PDF',
  ...
  if (canUseCachedPdf(currentRequest) && isTauri && __api) {
    try {
      ...
      var srcExists = false;
      try {
        var checkResult = await __api.call('check_path_exists', { path: _lastPdfPath });
      ...
      if (!srcExists) {
        ...
        var copyResult = await __api.call('copy_file', {
        ...
        if (copyResult && copyResult.success) {
          ...
          if (S.feat.autoOpenPdf && savePath) {
            try { __api.call('open_file', { path: savePath }); } catch(e) {}
          }
          return;
        }
      }
...
async function refreshPrinters() {
  if (!isTauri || !__api) { toast('仅在桌面模式下可用'); return; }
  try {
    var printers = await __api.call('get_printers');
