...
use crate::session::{Session, detect_file_type, is_allowed_extension};
...
async fn upload_files(
        ...
        .map_err(|e| AppError::BadRequest(format!("multipart 解析失败: {}", e)))?
    {
        ...
        file_infos.push(serde_json::json!({
            "name": filename.clone(),
            "path": filename,
...
async fn page_binary(
    ...
    let resolved = session.resolve_path(&pdf_path)
        .map_err(|e| AppError::BadRequest(e))?;
...
fn get_session(state: &AppState, session_id: &str) -> Result<Session, AppError> {
    if session_id.is_empty() {
        return Err(AppError::BadRequest("缺少 session ID".into()));
    }
