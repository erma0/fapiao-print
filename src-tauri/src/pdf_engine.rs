...
#[path = "pdf_engine_text_extract.rs"]
mod text_extract;
...
pub fn extract_pdf_text(pdf_path: &str, page_idx: u32) -> Result<PdfTextResult, String> {
    let result = text_extract::extract_pdf_text_with_pdf_oxide(pdf_path, page_idx)?;
    log::info!("pdf_oxide 文本提取成功: {} 字符, {} 行", result.text.len(), result.lines.len());
    Ok(result)
}
// =====================================================
...
/// A layout slot in mm coordinates (bottom-left origin, for PDF layout).
struct LayoutSlotMm {
    x_mm: f32,
    y_mm: f32,
    w_mm: f32,
...
/// Calculate layout slot positions in mm (bottom-left origin for PDF layout).
fn calculate_layout_mm(settings: &RenderSettings) -> (Vec<LayoutSlotMm>, f32, f32) {
    ...
    for r in 0..settings.rows as usize {
        for c in 0..settings.cols as usize {
            // Convert row from JS (top-down) to PDF (bottom-up)
...
pub fn generate_pdf_from_layout(
    ...
    // the lopdf hybrid path (images as JPEG XObjects) and the pdf_oxide fallback.
    ...
    // encoded as JPEG XObjects. Falls back to pdf_oxide pipeline on any error.
    match generate_pdf_passthrough(request, output_path, on_progress.as_ref(), &sources) {
        Ok(()) => return Ok(()),
        Err(e) => {
            log::warn!("lopdf直通失败，回退pdf_oxide渲染管道: {}", e);
            // Continue with pdf_oxide pipeline below
        }
    }
    ...
    let mut builder = pdf_oxide::writer::DocumentBuilder::new()
        .metadata(pdf_oxide::writer::DocumentMetadata::new().title("发票打印"));
    ...
    for (i, page_spec) in request.pages.iter().enumerate() {
        ...
        let mut content_elements: Vec<pdf_oxide::elements::ContentElement> = Vec::new();
        ...
        for (slot_idx, slot_spec) in page_spec.slots.iter().enumerate() {
            ...
            let (img_bytes, img_w, img_h) = match source {
                ImageSource::Decoded(img) => {
                    ...
                    if rotated.write_to(&mut cursor, image::ImageFormat::Png).is_err() {
                        continue;
                    }
                    (buf, rotated.width(), rotated.height())
                }
                ImageSource::JpegPassthrough { raw_bytes, width, height, .. } => {
                    ...
                    if rot == 90 || rot == 270 {
                        ...
                        (raw_bytes.clone(), *width, *height)
                    }
                }
            };

            let iw_mm = img_w as f32 * 25.4 / RENDER_DPI as f32;
            let ih_mm = img_h as f32 * 25.4 / RENDER_DPI as f32;

            let (mut scale_x, mut scale_y) = match request.settings.fit_mode.as_str() {
                "fill" => {
                ...
                "custom" => {
                    ...
                    let s = contain_s * request.settings.custom_scale;
                    (s, s)
                }
                _ => {
            ...
            if per_oy != 0.0 { offset_y_mm -= per_oy; }

            let x_pt = offset_x_mm * MM_TO_PT;
            let y_pt = offset_y_mm * MM_TO_PT;
            let w_pt = draw_w_mm * MM_TO_PT;
            let h_pt = draw_h_mm * MM_TO_PT;
            ...
            match pdf_oxide::elements::ImageContent::from_bytes(rect, img_bytes) {
                Ok(img_content) => {
                    content_elements.push(pdf_oxide::elements::ContentElement::Image(img_content));
                    page_has_content = true;
                }
                Err(e) => {
                    log::warn!("pdf_oxide fallback: ImageContent::from_bytes failed for slot {}: {}", slot_idx, e);
                }
            }
        }

        let has_overlay = request.settings.page_num || request.settings.print_date;
        if !page_has_content && !has_overlay {
            log::info!("Skipping empty page {}", i + 1);
            continue;
        }

        let mut page_builder = builder.page(pdf_oxide::writer::PageSize::Custom(pw_pt, ph_pt));

        if !content_elements.is_empty() {
            page_builder = page_builder.elements(content_elements);
        }

        if has_overlay {
            let mut overlay_parts = Vec::new();
            if request.settings.page_num {
                overlay_parts.push(format!("第 {} 页 / 共 {} 页", i + 1, request.pages.len()));
            }
            if request.settings.print_date {
                let now = std::time::SystemTime::now();
                let duration = now.duration_since(std::time::UNIX_EPOCH).unwrap_or_default();
                let (year, month, day) = days_to_ymd((duration.as_secs() / 86400) as i64);
                overlay_parts.push(format!("打印日期 {:04}-{:02}-{:02}", year, month, day));
            }
            let overlay_text = overlay_parts.join("  ");
            let text_y_pt = 5.0 * MM_TO_PT;
            page_builder = page_builder
                .at(pw_pt / 2.0, text_y_pt)
                .font("Helvetica", 9.0)
                .text(&overlay_text);
        }
    ...
    builder.save(output_path)
        .map_err(|e| format!("pdf_oxide保存失败: {}", e))?;
...
/// image/OFD pages are embedded as high-quality JPEG.
    ...
    for (page_idx, page_spec) in request.pages.iter().enumerate() {
        ...
        for (slot_idx, slot) in page_spec.slots.iter().enumerate() {
            ...
            let (xobj_id, src_w_pt, src_h_pt) = if let Some(pdf_path) = &file.pdf_path {
                ...
                // FlateDecode ensures lossless image quality — no JPEG quality loss.
