...
use pdf_engine::{FileData, RenderedPage, ComGuard, LayoutRenderRequest, PdfTextResult};
use platform::PrinterInfo;
#[cfg(feature = "ocr")]
...
/// Check if pdfium library is available for rendering/printing
#[command]
fn check_pdfium_available() -> bool {
    pdfium_bindings::find_pdfium_lib().is_some()
}

/// Cancel any in-progress download (pdfium.dll)
#[command]
fn cancel_download() {
...
async fn pdfium_vector_print(
    ...
    let resolved_printer = match printer_name {
        ...
        _ => platform::get_default_printer_name()
